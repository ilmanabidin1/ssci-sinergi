...
const demoValues: Partial<Values>[] = [
  { customerName: "Budi Santoso Demo", customerId: "3273010101010001", phone: "081234567890", email: "budi.demo@example.test", address: "Jl. Contoh Demo No. 1, Kota Bandung" },
  { businessName: "Usaha Demo SSCI", businessType: "Perdagangan Demo", businessAge: "24", monthlyRevenue: "25000000", monthlyExpenses: "15000000", existingDebt: "2000000", collateralValue: "100000000", requestedAmount: "50000000", financingTenor: "24", marginRate: "10", loanPurpose: "Modal kerja untuk pengembangan Usaha Demo SSCI" },
  { legalDocuments: [{ type: "KTP", status: "complete", notes: "Dokumen demo untuk pengujian" }, { type: "NPWP", status: "complete", notes: "Dokumen demo untuk pengujian" }, { type: "NIB", status: "complete", notes: "Dokumen demo untuk pengujian" }], businessShariaCompliant: "yes", shariaComplianceNotes: "Catatan kepatuhan syariah demo untuk pengujian" },
  { environmentalPractices: "Menggunakan kemasan ramah lingkungan demo", socialImpact: "Menyediakan lapangan kerja lokal demo", governanceQuality: "good" },
];
...
export default function NewApplication() {
  ...
  const fillDemo = (demoStep: number) => setValues(current => ({ ...current, ...demoValues[demoStep] } as Values));
  const fillAllDemo = () => setValues(current => demoValues.reduce<Values>((next, demo) => ({ ...next, ...demo }), current));
  ...
  return <div className="min-h-screen bg-gray-50"><nav className="border-b bg-white"><div className="container flex items-center justify-between py-4"><Button variant="ghost" size="sm" asChild><Link href="/"><ArrowLeft className="mr-2 h-4 w-4" />Kembali</Link></Button><div className="flex items-center gap-2"><Shield className="h-6 w-6 text-primary" /><b className="text-xl text-primary">SSCI</b></div><span className="text-sm text-gray-600">{user?.name || ""}</span></div></nav>
     <main className="container max-w-4xl py-6 sm:py-8"><div className="mb-6"><div className="flex flex-wrap items-start justify-between gap-3"><div><h1 className="text-3xl font-bold text-gray-900">Aplikasi Pembiayaan Baru</h1><p className="mt-2 text-gray-600">Lengkapi data nasabah untuk penilaian kelayakan pembiayaan</p></div><Button type="button" variant="outline" onClick={fillAllDemo}>Isi Semua Contoh</Button></div><p className="mt-3 text-xs text-muted-foreground">Data demo hanya untuk pengujian dan tidak akan dikirim otomatis.</p></div>
