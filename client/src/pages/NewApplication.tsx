...
import { AlertCircle, Check, CheckCircle2, Download, HelpCircle, Loader2 } from "lucide-react";
...
export default function NewApplication() {
  ...
  const murabahahInfo = <Alert className="border-[#cfd8e8] bg-[#eef2f8] text-navy-900"><HelpCircle className="h-4 w-4" /><AlertTitle>Panduan Akad Murabahah</AlertTitle><AlertDescription className="text-navy-900">Form ini mengacu pada OJK Pedoman Produk Murabahah dan Fatwa DSN-MUI No. 04/DSN-MUI/IV/2000. Akad Murabahah adalah akad jual beli di mana bank bertindak sebagai penjual dan nasabah sebagai pembeli, dengan margin keuntungan yang disepakati. Seluruh kolom bersifat opsional.</AlertDescription></Alert>;
  ...
  const murabahahPreviewPanel = <div className="rounded-lg border border-[#cfd8e8] bg-[#eef2f8] p-4"><div className="mb-2 text-sm font-medium text-navy-900">Perhitungan Piutang Murabahah</div>{murabahahPreviewQuery.isLoading ? <div className="flex items-center gap-2 text-sm text-navy-900"><Loader2 className="h-4 w-4 animate-spin" />Menghitung...</div> : previewData ? <><dl className="grid gap-2 text-sm sm:grid-cols-2"><div className="flex justify-between gap-3 border-b border-[#e1e8f4] pb-1"><dt className="text-navy-900">Harga perolehan</dt><dd className="font-medium text-navy-900">Rp {fmt(previewData.hargaPerolehan)}</dd></div><div className="flex justify-between gap-3 border-b border-[#e1e8f4] pb-1"><dt className="text-navy-900">Pokok pembiayaan</dt><dd className="font-medium text-navy-900">Rp {fmt(previewData.pokokPembiayaan)}</dd></div><div className="flex justify-between gap-3 border-b border-[#e1e8f4] pb-1"><dt className="text-navy-900">Margin nominal</dt><dd className="font-medium text-navy-900">{previewData.marginNominal != null ? `Rp ${fmt(previewData.marginNominal)}` : "—"}</dd></div><div className="flex justify-between gap-3"><dt className="font-semibold text-navy-900">Piutang Murabahah</dt><dd className="font-semibold text-navy-900">Rp {fmt(previewData.piutangMurabahah)}</dd></div></dl><div className="mt-3 flex flex-wrap items-center gap-2 text-xs text-navy-900">Tipe disarankan: <span className="rounded-full bg-navy-900 px-2 py-0.5 font-medium text-white">{previewData.suggestedType === "ultra_mikro" ? "Ultra Mikro (≤20jt)" : "Standard"}</span></div>{(previewData.wakalahForbidden || isPersonal) && <Alert className="mt-3 border-amber-300 bg-amber-50 text-amber-950"><AlertCircle className="h-4 w-4" /><AlertTitle>Perhatian</AlertTitle><AlertDescription className="text-amber-900">{(previewData.wakalahForbidden ? "Berdasarkan tipe akad, wakalah TIDAK boleh diberikan kepada nasabah. " : "")}{isPersonal ? "Personal Financing: bank TIDAK boleh memberi wakalah kepada nasabah; aset harus mudah dijual kembali." : ""}</AlertDescription></Alert>}</> : <p className="text-sm text-navy-900">Lengkapi harga pokok, biaya langsung, diskon pemasok, atau uang muka untuk menghitung Piutang Murabahah secara otomatis.</p>}</div>;
  const murabahahStep = <>
    {murabahahInfo}
    {section("1. Jenis & Objek Akad", <>
      ...
      {(values.murabahahType === "ultra_mikro" || values.murabahahType === "personal") && <Alert className="border-[#cfd8e8] bg-[#eef2f8] text-navy-900"><AlertCircle className="h-4 w-4" /><AlertTitle>Ultra Mikro / Akad Mu'allag</AlertTitle><AlertDescription className="text-navy-900">Akad efektif setelah nasabah melaporkan bukti pembelian (maks 15 hari).</AlertDescription></Alert>}
    </>)}
  ...
  const mudharabahStep = <>
    ...
    {section("1. Jenis & Prinsip", <>
      ...
      <p className="rounded-md bg-[#eef2f8] p-3 text-xs text-navy-900">Bank menyediakan 100% modal; keuntungan dibagi nisbah; kerugian finansial ditanggung bank kecuali karena kelalaian nasabah (ta'addi/taqshir/mukhalafah).</p>
    </>)}
  ...
  const multijasaStep = <>
    <Alert className="border-[#cfd8e8] bg-[#eef2f8] text-navy-900">
      <HelpCircle className="h-4 w-4" />
  ...
  return <div className="min-h-screen bg-ivory"><AppHeader />
            ...
            <div className="space-y-4">
              <div className="grid gap-3 sm:grid-cols-2">
                {BPRS_PRODUCT_SEGMENTS.map(segKey => {
                  ...
                  const active = values.productSegment === segKey;
                  return (
                    <button
                      key={segKey}
                      type="button"
                      onClick={() => {
                        const requiredDocs = seg.requiredDocuments.map(type => ({
                          type,
                        ...
                        setValues(v => ({
                          ...v,
                          productSegment: segKey,
                          legalDocuments: requiredDocs,
                        }));
                      }}
                    ...
                    >
                      ...
                      <div className="mt-3 flex flex-wrap gap-2 text-[11px]">
                        <span className="rounded bg-slate-100 px-2 py-0.5 font-medium text-slate-700">
                          Maks DSR: {seg.defaultMaxDsr}%
                        </span>
                        <span className="rounded bg-slate-100 px-2 py-0.5 font-medium text-slate-700">
                          {seg.requiredDocuments.length} Dokumen Wajib
                        </span>
                      </div>
                    </button>
                  );
              ...
              <div className="rounded-lg border border-[#cfd8e8] bg-[#eef2f8] p-4 text-xs text-navy-900">
                <div className="font-semibold text-navy-900 mb-1">
                  Ketentuan Jaminan & Dokumen untuk Segmen: {BPRS_SEGMENT_DETAILS[values.productSegment]?.label}
                </div>
